...
// a key-value list of columns.
  ...
  // Queries a single row by its ID.
  Future<List<Map<String, dynamic>>> queryRowById(int id) async {
    return await _db.query(
      table,
      where: '$columnId = ?',
      whereArgs: [id],
      // You might use .where: '$columnId = $id', but whereArgs is safer against SQL injection
    );
  }
...
// returned. This should be 1 as long as the row exists.
  ...
  Future<int> deleteAllRows() async {
    // Deleting all rows where the condition is '1' (which is always true)
    return await _db.delete(table);
    // Alternative: return await _db.rawDelete('DELETE FROM $table');
  }
}